...
import { listContractFiles, addContractFile, deleteContractFile } from '@/api/contract/details/contract_file' // adjust path if you named folder differently

export default function FileManagerListPage() {
  const params = useParams()
  const contractId = params.id

  console.log('Route Contract ID:', contractId) // ⭐ Paste here
  ...
  const loadFiles = async () => {
    try {
      ...
      const formatted = data.map(item => ({
        ...
        name: item.name || '',
        fileUrl: item.file_name, // full URL from server
  ...
  const handleUpload = async () => {
    ...
    // API probably expects these fields; adjust keys if backend expects different names
  ...
  const handleDownload = row => {
    // If API returns full URL in fileUrl, just open it
    if (row.fileUrl) {
    ...
    <Box className='mt-2'>
      <Card sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <GlobalButton variant='contained' color='primary' startIcon={<RefreshIcon />} onClick={loadFiles}>
              Refresh
            </GlobalButton>
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <GlobalButton variant='contained' startIcon={<AddIcon />} onClick={() => setDrawerOpen(true)}>
              Upload File
            </GlobalButton>
          </Box>
        </Box>
        ...
        <div className='overflow-x-auto'>
          <table className={styles.table}>
            <thead>
              <tr>
                {['#', 'Action', 'File Name', 'Last Updated'].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
      ...
      >
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              {selectedFile && (
                <Typography sx={{ mt: 2, fontSize: '0.85rem', color: 'primary.main' }}>{selectedFile}</Typography>
              )}
            </Box>
        ...
        <Box mt={4} display='flex' gap={2}>
          <GlobalButton fullWidth onClick={handleUpload}>
            Upload
          </GlobalButton>
