...
import { incidentSchema } from '@/validations/incident.schema'

import { getIncidentList, addIncident, updateIncident, deleteIncident, getIncidentDetails } from '@/api/master/incidents'
...
const IncidentPageContent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (result.success || Array.isArray(dataArray)) {
        ...
        showToast('error', result.message || 'Failed to load incidents')
        setRows([])
      }
    } catch (err) {
      console.error(err)
  ...
  const handleEdit = async row => {
    ...
    setLoading(true)
    try {
      const result = await getIncidentDetails(row.id)
  ...
  const onSubmit = async data => {
    ...
    try {
      ...
      const result = isEdit ? await updateIncident({ ...payload, id: editId }) : await addIncident(payload)
      ...
      console.error(err)
      showToast('error', 'Failed to save Incident')
    } finally {
      setLoading(false)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    setLoading(true)
    try {
      const result = await deleteIncident(deleteDialog.row.id)
      if (result.success) {
        showToast('success', result.message)
        await loadData()
      } else {
      ...
      columnHelper.display({
        ...
        cell: info => (
          <Box sx={{ display: 'flex', gap: 1 }}>
            {canAccess('Incident', 'update') && (
              <IconButton size='small' color='primary' onClick={() => handleEdit(info.row.original)}>
                <i className='tabler-edit ' />
              </IconButton>
            )}
            {canAccess('Incident', 'delete') && (
              <IconButton
                size='small'
                color='error'
      ...
      header={
        <Box sx={{ mb: 2 }}>
          <Breadcrumbs aria-label='breadcrumb' sx={{ mb: 2 }}>
            ...
            <Typography color='text.primary'>Incidents</Typography>
          </Breadcrumbs>
        </Box>
      }
    >
        ...
        <Box sx={{ p: 5, display: 'flex', flexDirection: 'column', maxHeight: '100%' }}>
          <Box display='flex' justifyContent='space-between' alignItems='center' mb={3}>
            <Typography variant='h5' fontWeight={600}>
              {isEdit ? 'Edit Incident' : 'Add Incident'}
            </Typography>
...
export default function IncidentPage() {
  return (
    <PermissionGuard permission='Incident'>
      <IncidentPageContent />
    </PermissionGuard>
  )
}
