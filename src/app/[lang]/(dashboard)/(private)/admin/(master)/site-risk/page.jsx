...
import { siteRiskSchema } from '@/validations/siteRisk.schema'

import { getSiteRiskList, addSiteRisk, updateSiteRisk, deleteSiteRisk, getSiteRiskDetails } from '@/api/master/siterisk'
...
const SiteRiskPageContent = () => {
  ...
  const {
    ...
    resolver: zodResolver(siteRiskSchema),
    defaultValues: {
      name: '',
      description: '',
      status: 1
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (result.success || Array.isArray(dataArray)) {
        ...
        showToast('error', result.message || 'Failed to load site risks')
        setRows([])
      }
    } catch (err) {
      console.error(err)
      showToast('error', 'Failed to load site risks')
    } finally {
      setLoading(false)
    }
  }
  ...
  const onSubmit = async data => {
    ...
    try {
      ...
      const result = isEdit ? await updateSiteRisk({ ...payload, id: editId }) : await addSiteRisk(payload)
      ...
      console.error(err)
      showToast('error', 'Failed to save Site Risk')
    } finally {
      setLoading(false)
    }
  }
      ...
      columnHelper.display({
        ...
        cell: info => (
          <Box sx={{ display: 'flex', gap: 1 }}>
            {canAccess('Site Risk', 'update') && (
              <IconButton size='small' color='primary' onClick={() => handleEdit(info.row.original)}>
                <i className='tabler-edit ' />
              </IconButton>
            )}
            {canAccess('Site Risk', 'delete') && (
              <IconButton
                size='small'
                color='error'
      ...
      >
        ...
        <DialogContent sx={{ px: 5, pt: 1 }}>
          <Typography sx={{ color: 'text.secondary', fontSize: 14, lineHeight: 1.6 }}>
            ...
            <strong style={{ color: '#d32f2f' }}>{deleteDialog.row?.name || 'this record'}</strong>
            ?
            <br />
