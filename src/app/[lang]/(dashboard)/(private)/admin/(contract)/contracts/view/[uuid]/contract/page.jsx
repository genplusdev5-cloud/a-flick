...
export default function ContractViewPage({ contract }) {
  ...
  // -----------------------------
  // -----------------------------
  // Load Contract (From Prop or Fetch)
  // -----------------------------
  useEffect(() => {
    const initData = async () => {
      // 1. Use Prop if available (Priority)
      if (contract) {
        populateForm(contract)
        return
      }

      // 2. Fetch if missing (Fallback for standalone page)
      if (uuid) {
        try {
