...
import { getContractDetails } from '@/api/contract_group/contract'
...
export default function Project() {
  ...
  const handleAgreementPrint = async () => {
    ...
    const { getAgreementPDF } = await import('@/api/contract_group/contract/agreement')
  ...
  const handleSchedulePrint = async () => {
    ...
    const { getSchedulePDF } = await import('@/api/contract_group/contract/schedulepdf')
  ...
  if (error) {
    return (
      <Box sx={{ p: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        <Typography variant='body1'>UUID: {uuid}</Typography>
        <Typography variant='body2' color='text.secondary'>
          Please check if the contract exists and you have permission to view it.
        </Typography>
      </Box>
    )
  }
